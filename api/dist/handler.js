// src/insights/buildInsights.ts
function asArray(value) {
  return Array.isArray(value) ? value : value != null ? [value] : [];
}
function extractResources(input) {
  const out = [];
...
function dateValue(resource) {
  return resource?.effectiveDateTime ?? resource?.effectivePeriod?.end ?? resource?.effectivePeriod?.start ?? resource?.issued ?? resource?.meta?.lastUpdated;
}
...
function codeLabel(code, fallback) {
  if (!code)
    return fallback;
  if (typeof code?.text === "string" && code.text.trim())
...
function conditionText(condition) {
  if (typeof condition?.code?.text === "string" && condition.code.text.trim())
    return condition.code.text.trim();
...
function medicationChange(status) {
  const s = String(status ?? "").toLowerCase();
  if (!s)
    return void 0;
  if (s === "stopped" || s === "completed" || s === "entered-in-error")
    return "stopped";
  if (s === "active" || s === "in-progress" || s === "intended")
    return "started";
  return void 0;
}
function medicationDosageText(resource) {
...
function medicationPatientInstruction(resource) {
  const doseArr = asArray(resource?.dosage);
  const instrArr = asArray(resource?.dosageInstruction);
  const d = (doseArr.length > 0 ? doseArr : instrArr)[0];
...
function observationValue(obs) {
  ...
  if (typeof obs?.valueBoolean === "boolean")
    return obs.valueBoolean ? "Positive" : "Negative";
  if (obs?.valueCodeableConcept)
    return codeLabel(obs.valueCodeableConcept, "Result available");
  if (typeof obs?.valueInteger === "number")
    return String(obs.valueInteger);
  if (typeof obs?.valueDateTime === "string" && obs.valueDateTime)
    return obs.valueDateTime;
  return "Result available";
}
function hasConcreteObservationValue(obs) {
  return obs?.valueQuantity?.value != null || typeof obs?.valueString === "string" && obs.valueString.trim().length > 0 || typeof obs?.valueBoolean === "boolean" || obs?.valueCodeableConcept != null || typeof obs?.valueInteger === "number" || typeof obs?.valueDateTime === "string" && obs.valueDateTime.length > 0;
}
function observationFlag(obs) {
  const code = String(obs?.interpretation?.[0]?.coding?.[0]?.code ?? "").toUpperCase();
  if (!code)
    return void 0;
  if (code === "H" || code === "HH" || code === "HX")
    return "H";
  if (code === "L" || code === "LL" || code === "LX")
    return "L";
  if (code === "A" || code === "ABN")
    return "A";
  if (code === "CRIT" || code === "AA")
    return "critical";
  return void 0;
}
function refId(reference, expectedType) {
...
function isLabObservation(obs) {
  ...
  for (const cat of categories) {
    for (const coding of asArray(cat?.coding)) {
      if (String(coding?.code ?? "").toLowerCase() === "laboratory")
        return true;
    }
    if (String(cat?.text ?? "").toLowerCase().includes("lab"))
      return true;
  }
  return false;
}
function isSocialObservation(obs) {
  const categories = asArray(obs?.category);
  for (const cat of categories) {
    for (const coding of asArray(cat?.coding)) {
      if (String(coding?.code ?? "").toLowerCase() === "social-history")
        return true;
    }
    if (String(cat?.text ?? "").toLowerCase().includes("social"))
      return true;
  }
  const codeText = String(obs?.code?.text ?? "").toLowerCase();
  if (codeText.includes("smok") || codeText.includes("tobacco") || codeText.includes("alcohol"))
    return true;
  for (const coding of asArray(obs?.code?.coding)) {
    const display = String(coding?.display ?? "").toLowerCase();
    if (display.includes("smok") || display.includes("tobacco") || display.includes("alcohol"))
      return true;
  }
  return false;
}
function isMentalObservation(obs) {
  if (isPhq9Observation(obs))
    return true;
  const categories = asArray(obs?.category);
  for (const cat of categories) {
    if (String(cat?.text ?? "").toLowerCase().includes("mental"))
      return true;
    for (const coding of asArray(cat?.coding)) {
  ...
  const codeText = String(obs?.code?.text ?? "").toLowerCase();
  if (codeText.includes("mental") || codeText.includes("depression") || codeText.includes("anxiety"))
    return true;
  for (const coding of asArray(obs?.code?.coding)) {
    const display = String(coding?.display ?? "").toLowerCase();
    if (display.includes("mental") || display.includes("depression") || display.includes("anxiety"))
      return true;
  }
  return false;
}
function isPhq9Observation(obs) {
  const codeText = String(obs?.code?.text ?? "").toLowerCase();
  if (codeText.includes("phq-9") || codeText.includes("phq 9"))
    return true;
  for (const coding of asArray(obs?.code?.coding)) {
    ...
    if (code === "44261-6")
      return true;
    if (display.includes("phq-9") || display.includes("phq 9"))
      return true;
  }
  return false;
}
var VITAL_LOINC_CODES = /* @__PURE__ */ new Set([
  "85354-9",
...
function isVitalObservation(obs) {
  ...
  for (const cat of categories) {
    for (const coding of asArray(cat?.coding)) {
      const code2 = String(coding?.code ?? "").toLowerCase();
      if (code2 === "vital-signs" || code2 === "vitals")
        return true;
    }
    if (String(cat?.text ?? "").toLowerCase().includes("vital"))
      return true;
  }
  const code = obs?.code;
  for (const coding of asArray(code?.coding)) {
    ...
    if (VITAL_LOINC_CODES.has(c))
      return true;
    if (d.includes("blood pressure") || d.includes("bp systolic") || d.includes("bp diastolic") || d.includes("heart rate") || d.includes("body temperature") || d.includes("respiratory rate") || d.includes("oxygen saturation") || d === "weight" || d === "height" || d.includes("body mass index") || d.includes("bmi")) {
      return true;
    }
  }
  const text = String(code?.text ?? "").toLowerCase();
  if (text.includes("blood pressure") || text.includes("heart rate") || text.includes("temperature") || text.includes("respiratory rate") || text.includes("oxygen saturation") || text === "weight" || text === "height" || text.includes("body mass index") || text.includes("bmi")) {
    return true;
  }
  return false;
}
...
function isChronicProblemResource(condition) {
  if (!condition || condition.resourceType !== "Condition")
    return false;
  if (containsAnyTerm(condition?.code?.text, CHRONIC_PROBLEM_TERMS))
    return true;
  for (const coding of asArray(condition?.code?.coding)) {
    if (containsAnyTerm(coding?.display, CHRONIC_PROBLEM_TERMS))
      return true;
  }
  return false;
}
function isChronicMedicationResource(resource, medRef) {
  if (!resource)
    return false;
  if (containsAnyTerm(resource?.medicationCodeableConcept?.text, CHRONIC_MED_TERMS))
    return true;
  for (const coding of asArray(resource?.medicationCodeableConcept?.coding)) {
    if (containsAnyTerm(coding?.display, CHRONIC_MED_TERMS))
      return true;
  }
  if (containsAnyTerm(medRef?.code?.text, CHRONIC_MED_TERMS))
    return true;
...
function observationAbnormalLabel(obs) {
  const interp = String(obs?.interpretation?.[0]?.coding?.[0]?.code ?? "").toUpperCase();
  if (interp === "H" || interp === "HH" || interp === "HX")
    return "High";
  if (interp === "L" || interp === "LL" || interp === "LX")
    return "Low";
  if (interp === "A" || interp === "ABN")
    return "Abnormal";
  if (interp === "CRIT" || interp === "AA")
    return "Critical";
  ...
  if (Number.isFinite(value) && Number.isFinite(low) && value < low)
    return "Low";
  if (Number.isFinite(value) && Number.isFinite(high) && value > high)
    return "High";
  return void 0;
}
function encounterClassCode(encounter) {
  return String(encounter?.class?.code ?? encounter?.class?.display ?? "").trim().toUpperCase();
}
function daysAgo(days) {
  return Date.now() - days * 24 * 60 * 60 * 1e3;
}
function buildInsights(bundle, opts = {}) {
  const entries = Array.isArray(bundle.entry) ? bundle.entry : [];
  const resources = {};
  const extracted = extractResources(entries.length > 0 ? entries : bundle);
  for (const r of extracted) {
    if (r?.resourceType && r?.id)
      resources[`${r.resourceType}/${r.id}`] = r;
  }
  const patientRes = Object.values(resources).find((r) => r.resourceType === "Patient");
  const patientName = patientRes?.name?.[0] ? `${patientRes.name[0].given?.join(" ") ?? ""} ${patientRes.name[0].family ?? ""}`.trim() : "Unknown Patient";
  const patient = {
    id: patientRes?.id ?? "unknown",
    name: patientName,
    dob: patientRes?.birthDate,
    sex: patientRes?.gender,
    identifiers: Array.isArray(patientRes?.identifier) ? patientRes.identifier.map((i) => ({ system: i.system, value: i.value })) : []
  };
  const problems = Object.values(resources).flatMap((resource) => {
  ...
  }).sort((a, b) => dateScore(b.at) - dateScore(a.at)).map(({ text, status, onset, evidence }) => ({ text, status, onset, evidence }));
  const procedures = Object.values(resources).flatMap((resource) => {
    if (resource?.resourceType !== "Procedure" || !resource?.id)
      return [];
    const at = resource?.performedDateTime ?? resource?.performedPeriod?.end ?? resource?.performedPeriod?.start ?? resource?.meta?.lastUpdated;
    return [
      {
        text: codeLabel(resource?.code, "Procedure"),
        status: typeof resource?.status === "string" ? resource.status : void 0,
        at,
        evidence: [{ resourceType: "Procedure", id: resource.id }]
      }
    ];
  }).sort((a, b) => dateScore(b.at) - dateScore(a.at)).map(({ text, status, at, evidence }) => ({ text, status, at, evidence }));
  const meds = Object.values(resources).flatMap((resource) => {
    ...
    const medRef = medRefId ? resources[`Medication/${medRefId}`] : void 0;
    const text = codeLabel(resource?.medicationCodeableConcept, "") || codeLabel(medRef?.code, "") || resource?.medicationReference?.display || "Medication";
    const status = typeof resource?.status === "string" ? resource.status : void 0;
    const dosage = medicationDosageText(resource);
    const patientInstruction = medicationPatientInstruction(resource);
    const at = resource?.effectiveDateTime ?? resource?.effectivePeriod?.start ?? resource?.effectivePeriod?.end ?? resource?.dateAsserted ?? resource?.authoredOn ?? resource?.meta?.lastUpdated;
    return [
      {
        text,
        dosage,
  ...
  }).sort((a, b) => dateScore(b.at) - dateScore(a.at)).reduce((acc, med) => {
    if (acc.some((m) => m.text === med.text && m.dosage === med.dosage && m.patientInstruction === med.patientInstruction))
      return acc;
  ...
  const allergies = Object.values(resources).flatMap((resource) => {
    if (resource?.resourceType !== "AllergyIntolerance" || !resource?.id)
      return [];
    return [
      {
        text: codeLabel(resource?.code, "Allergy"),
        criticality: typeof resource?.criticality === "string" ? resource.criticality : typeof resource?.reaction?.[0]?.severity === "string" ? resource.reaction[0].severity : void 0,
        at: resource?.onsetDateTime ?? resource?.recordedDate ?? resource?.meta?.lastUpdated,
        evidence: [{ resourceType: "AllergyIntolerance", id: resource.id }]
      }
    ];
  }).sort((a, b) => dateScore(b.at) - dateScore(a.at)).map(({ text, criticality, evidence }) => ({ text, criticality, evidence }));
  const socialHistory = Object.values(resources).flatMap((resource) => {
    if (resource?.resourceType !== "Observation" || !resource?.id || !isSocialObservation(resource))
      return [];
    return [
      {
  ...
  }).sort((a, b) => dateScore(b.at) - dateScore(a.at)).map(({ label, latest, at, evidence }) => ({ label, latest, at, evidence }));
  const mentalStatus = Object.values(resources).flatMap((resource) => {
    if (resource?.resourceType !== "Observation" || !resource?.id || !isMentalObservation(resource))
      return [];
    return [
      {
  ...
  const immunizations = Object.values(resources).flatMap((resource) => {
    if (resource?.resourceType !== "Immunization" || !resource?.id)
      return [];
    const at = resource?.occurrenceDateTime ?? resource?.recorded ?? resource?.meta?.lastUpdated;
    return [
      {
  ...
  }).sort((a, b) => dateScore(b.at) - dateScore(a.at)).map(({ vaccine, status, at, evidence }) => ({ vaccine, status, at, evidence }));
  const vitals = Object.values(resources).flatMap((resource) => {
    if (resource?.resourceType !== "Observation" || !resource?.id || !isVitalObservation(resource))
      return [];
    return [
      {
  ...
  }).sort((a, b) => dateScore(b.at) - dateScore(a.at)).reduce((acc, current) => {
  ...
  const labs = Object.values(resources).flatMap((resource) => {
    if (!resource?.resourceType || !resource?.id)
      return [];
    ...
    if (resource.resourceType === "DiagnosticReport") {
      const resultRows = asArray(resource?.result).flatMap((resultRef) => {
    ...
    return [];
  }).sort((a, b) => dateScore(b.at) - dateScore(a.at)).map(({ label, latest, flag, evidence }) => ({ label, latest, flag, evidence }));
  const timeline = Object.values(resources).flatMap((resource) => {
    if (!resource?.resourceType || !resource?.id)
      return [];
      ...
      return [
        {
          ...
          summary: resource?.status ? `Status: ${resource.status}` : void 0,
          evidence: [{ resourceType: "Encounter", id: resource.id }]
        }
      ];
    }
    if (resource.resourceType === "DiagnosticReport") {
      const resultFlags = asArray(resource?.result).flatMap((resultRef) => {
        const observationId = refId(resultRef?.reference, "Observation");
        if (!observationId)
          return [];
        const observation = resources[`Observation/${observationId}`];
      ...
      const severity = resultFlags.includes("critical") ? "critical" : resultFlags.length > 0 ? "high" : void 0;
      return [
        {
          at: dateValue(resource) ?? "Unknown date",
          kind: "lab",
          label: codeLabel(resource.code, "Diagnostic report"),
          summary: severity ? `Abnormal result (${resultFlags[0]})` : resource?.status ? `Status: ${resource.status}` : void 0,
          severity,
    ...
    if (resource.resourceType === "Procedure") {
      return [
        {
          at: resource?.performedDateTime ?? resource?.performedPeriod?.end ?? resource?.performedPeriod?.start ?? resource?.meta?.lastUpdated ?? "Unknown date",
          kind: "procedure",
  ...
  const encounters = Object.values(resources).filter((r) => r?.resourceType === "Encounter" && r?.id);
  const encountersLast12m = encounters.filter((e) => {
    const at = dateScore(e?.period?.start ?? e?.period?.end ?? e?.meta?.lastUpdated);
    return at > 0 && at >= daysAgo(365);
  });
  const ed12m = encountersLast12m.filter((e) => encounterClassCode(e).includes("EMER")).length;
  const ip12m = encountersLast12m.filter((e) => {
    const c = encounterClassCode(e);
    return c.includes("IMP") || c.includes("INPAT");
  }).length;
  const utilizationEvidence = encountersLast12m.slice(0, 10).map((e) => ({ resourceType: "Encounter", id: e.id }));
  const abnormalLabs = Object.values(resources).filter((r) => r?.resourceType === "Observation" && r?.id && isLabObservation(r)).map((obs) => ({
    id: obs.id,
  ...
  })).filter((row) => !!row.abnormal).sort((a, b) => dateScore(b.at) - dateScore(a.at));
  const abnormalVitals = Object.values(resources).filter((r) => r?.resourceType === "Observation" && r?.id && isVitalObservation(r)).map((obs) => ({
    id: obs.id,
  ...
  const nonFinalObservations = Object.values(resources).filter((r) => r?.resourceType === "Observation" && r?.id && (isLabObservation(r) || isVitalObservation(r))).filter((obs) => {
    const s = String(observationStatus(obs) ?? "").toLowerCase();
    return !!s && s !== "final";
  });
  const severeAllergies = Object.values(resources).filter((r) => r?.resourceType === "AllergyIntolerance" && r?.id).filter((a) => {
  ...
  const chronicProblems = Object.values(resources).filter((r) => r?.resourceType === "Condition" && r?.id && isChronicProblemResource(r)).map((c) => ({
    id: c.id,
    text: conditionText(c),
    at: c?.onsetDateTime ?? c?.recordedDate ?? c?.meta?.lastUpdated
  })).sort((a, b) => dateScore(b.at) - dateScore(a.at));
  const chronicMeds = Object.values(resources).filter((r) => (r?.resourceType === "MedicationStatement" || r?.resourceType === "MedicationRequest") && r?.id).filter((m) => {
  ...
  const recentAcuteEncounters = encounters.map((e) => ({
    id: e.id,
  ...
  })).filter((e) => dateScore(e.at) >= daysAgo(30)).filter((e) => e.classCode.includes("EMER") || e.classCode.includes("IMP") || e.classCode.includes("INPAT")).sort((a, b) => dateScore(b.at) - dateScore(a.at));
  const banners = [];
  ...
  if (chronicProblems.length > 0 || chronicMeds.length > 0) {
    ...
    const evidence = [
      ...chronicProblems.slice(0, 6).map((p) => ({ resourceType: "Condition", id: p.id })),
  ...
  return {
    ...
    timeline,
    resources: opts.includeResources ? resources : void 0
  };
}
