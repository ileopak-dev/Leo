#!/usr/bin/env python3
"""
PHI Sanitization Tool
Sanitizes HL7 and CCD files to remove PHI while maintaining context
"""
import os
import sys
import argparse
...
import config
from src.sanitizer import PHISanitizer
...
def get_input_files(input_dir: str, count: int = None, random_select: bool = False,
    ...
    # Convert to strings and filter out hidden files/directories and CCD directory
    all_files = [str(f) for f in all_files
                 if not any(part.startswith('.') for part in Path(f).parts)
                 and 'CCD' not in Path(f).parts]  # Exclude CCD directory (HL7 v3 XML files)
...
def generate_output_filename(input_path: str, message_type: str, input_dir: str) -> str:
    """
    Generate output filename with message type prefix
    Preserves subdirectory structure relative to input_dir
    Format: {subdirs}/{MESSAGE_TYPE}_{original_filename}
    """
    ...
    # Clean message type for filename (replace ^ with _)
    clean_msg_type = message_type.replace('^', '_')
    
    # Create new filename with message type prefix
    new_filename = f"{clean_msg_type}_{filename}"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Sanitize HL7/CCD files to remove PHI while maintaining context'
    )
    ...
    logger.info("=" * 60)
    logger.info("PHI SANITIZATION TOOL")
    logger.info("=" * 60)
    ...
    # Clean up output folders for message types included in this run.
    target_output_subdirs = set()
    for input_path in input_files:
        rel_path = os.path.relpath(input_path, config.INPUT_DIR)
        parts = Path(rel_path).parts
        if parts:
            target_output_subdirs.add(parts[0])
    cleanup_previous_run(output_subdirs=sorted(target_output_subdirs))
    ...
    sanitizer = PHISanitizer(config.DB_FILE)
    ...
    for i, input_path in enumerate(input_files, 1):
        ...
        try:
            result = sanitizer.sanitize_file(input_path, "temp_output.txt")
            ...
            os.rename("temp_output.txt", output_path)
